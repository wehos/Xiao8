    ...
    <el-dialog v-model="editorOpen" :title="t(editingSlot ? 'modelApi.editSlot' : 'modelApi.addSlot')" width="640px" top="5vh" class="model-slot-dialog" :close-on-click-modal="false" :close-on-press-escape="!saving" :show-close="!saving" @closed="clearEditor">
      <el-form label-position="top" @submit.prevent="saveSlot">
        ...
        <el-form-item :label="t('modelApi.key')">
          <el-radio-group v-if="editingSlot?.api_key" v-model="form.keyMode" :disabled="saving"><el-radio value="keep">{{ t('modelApi.keepKey') }}</el-radio><el-radio value="replace">{{ t('modelApi.replaceKey') }}</el-radio><el-radio value="clear">{{ t('modelApi.clearKey') }}</el-radio></el-radio-group>
          <el-input v-if="form.keyMode === 'replace'" v-model="form.api_key" data-testid="slot-key" type="password" show-password autocomplete="new-password" :placeholder="t('modelApi.keyPlaceholder')" :disabled="saving" />
          <p v-if="form.keyMode === 'clear'" class="field-hint">{{ t('modelApi.clearKeyHint') }}</p>
...
import { modelSlotForm, modelSlotPayload, needsModelKeyUpdate } from './model-api-form'
